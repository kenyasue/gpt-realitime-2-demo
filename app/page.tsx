...
import { Stage, type SessionState } from "./components/Stage";
...
export default function Page() {
  ...
  // True when the next `session.created` event should auto-trigger a `response.create`
  // so the assistant speaks first (used by personas with autoStart, e.g. Survey).
  ...
  /* ────────────────────────────────────────────────────────────
   * Event handler — transcript + interruption logic
   * ──────────────────────────────────────────────────────────── */
  const handleEvent = useCallback((event: ServerEvent) => {
    if (process.env.NODE_ENV !== "production") {
      // Verbose protocol debugging — only in dev
    ...
    switch (event.type) {
      ...
      case "input_audio_buffer.speech_started": {
        ...
        // Barge-in: if assistant is still speaking, mark its turn interrupted
        setTurns((prev) =>
          prev.map((t) =>
      ...
      case "conversation.item.input_audio_transcription.completed": {
        ...
        setTurns((prev) => {
          ...
          if (idx === -1) {
            // No matching speech_started turn — append a new one
            return [
              ...prev,
      ...
      case "conversation.item.input_audio_transcription.failed": {
        ...
        setTurns((prev) =>
          prev.map((t) =>
            t.id === ev.item_id ? { ...t, final: true } : t,
          ),
        );
        break;
      }
  ...
  async function startSession(overrides?: StartOverrides) {
    ...
    try {
      ...
      // Dev-only debug hook (used by scripts/e2e-voice-test.mjs)
      ...
      // PTT mode was already baked into the initial session via /api/session.
      // No additional session.update needed here.

      startTimer();
      setSession("live");

      // Save unsubscribe on client for cleanup
  ...
  /* ────────────────────────────────────────────────────────────
   * Persona / voice / mode change handlers
   *
   * Persona and voice changes RESTART the session and clear the transcript:
   * the GA Realtime API doesn't allow voice to change mid-session, so any
   * new selection requires a fresh ephemeral key + handshake.
   * ──────────────────────────────────────────────────────────── */
  ...
  /* ────────────────────────────────────────────────────────────
   * Push-to-talk handlers (button + Space key)
   * ──────────────────────────────────────────────────────────── */
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (e.code === "Space" && session === "live" && mode === "ptt") {
        // Don't hijack space inside text inputs / buttons (we have no inputs but be defensive)
